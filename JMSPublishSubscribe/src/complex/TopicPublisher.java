...
public class TopicPublisher implements MessageListener {
	...
	for (int i = 0; i < messages; i++) {  // this effectively:  batch * messages = 20,000,000; look at line 83
	    publisher.send(msg);
	    if ((i + 1) % 1000 == 0) {
    ...
    /**
     * Update mutex through control topic subscription
     * 
     * control = session.createTopic("topictest.control");  line 70
     * 
     * session.createConsumer(control).setMessageListener(this);  line 79
     */
    @Override
